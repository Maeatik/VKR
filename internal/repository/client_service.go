...
func (c *ClientServicePostgres) GetListSites(ctx context.Context, userID int) ([]v1.Site, error) {
	var sites []v1.Site

	query := `SELECT "url", tag from "Site" WHERE "user_id(FK)"=$1`
	...
	for rows.Next() {
		var site v1.Site
		if err := rows.Scan(&site.Url, &site.Tag); err != nil {
